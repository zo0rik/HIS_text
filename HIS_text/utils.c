...
// ------ 数据加载与保存 (已适配新增的记录与病床字段) ------
void loadAllDataFromTxt() {
    FILE* fp = fopen("patients.txt", "r");
    if (fp) {
        Patient* tail = patientHead; Patient temp;
        while (fscanf(fp, "%19s %49s %99s %99s %d %99s %d %lf",
            ...
            Patient* newNode = (Patient*)malloc(sizeof(Patient)); *newNode = temp; newNode->next = NULL;
    ...
    if (fp) {
        Medicine* tail = medicineHead; Medicine temp;
        while (fscanf(fp, "%19s %99s %d %lf %99s",
            temp.id, temp.name, &temp.stock, &temp.price, temp.expiryDate) == 5) {
            Medicine* newNode = (Medicine*)malloc(sizeof(Medicine)); *newNode = temp; newNode->next = NULL;
            tail->next = newNode; tail = newNode;
        }
        fclose(fp);
    }
    ...
    if (fp) {
        Staff* tail = staffHead; Staff temp;
        while (fscanf(fp, "%19s %49s %99s %99s %99s",
            temp.id, temp.password, temp.name, temp.department, temp.level) == 5) {
            Staff* newNode = (Staff*)malloc(sizeof(Staff)); *newNode = temp; newNode->next = NULL;
            tail->next = newNode; tail = newNode;
        }
        fclose(fp);
    }
    ...
    if (fp) {
        Record* tail = recordHead; Record temp;
        // 适配新增的 createTime
        while (fscanf(fp, "%29s %d %19s %19s %lf %d %299s %29s",
            ...
            Record* newNode = (Record*)malloc(sizeof(Record)); *newNode = temp; newNode->next = NULL;
            tail->next = newNode; tail = newNode;
        }
        fclose(fp);
    }
    ...
    if (fp) {
        ...
        // 适配新的病床字典
        while (fscanf(fp, "%d %d %19s %49s %49s %lf",
            &temp.bedNumber, &temp.isOccupied, temp.patientId, temp.wardType, temp.bedType, &temp.price) == 6) {
...
void saveAllDataToTxt() {
    ...
    if (fp) {
        ...
        while (p) { fprintf(fp, "%s %s %s %s %d %s %d %.2f\n", p->id, p->password, p->name, p->gender, p->age, p->allergy, p->isEmergency, p->balance); p = p->next; }
        fclose(fp);
    }
    fp = fopen("medicines.txt", "w");
    if (fp) {
        Medicine* m = medicineHead->next;
        while (m) { fprintf(fp, "%s %s %d %.2f %s\n", m->id, m->name, m->stock, m->price, m->expiryDate); m = m->next; }
        fclose(fp);
    }
    fp = fopen("staff.txt", "w");
    if (fp) {
        Staff* s = staffHead->next;
        while (s) { fprintf(fp, "%s %s %s %s %s\n", s->id, s->password, s->name, s->department, s->level); s = s->next; }
        fclose(fp);
    }
    fp = fopen("records.txt", "w");
    if (fp) {
        Record* r = recordHead->next;
        while (r) { fprintf(fp, "%s %d %s %s %.2f %d %s %s\n", r->recordId, r->type, r->patientId, r->staffId, r->cost, r->isPaid, r->description, r->createTime); r = r->next; }
        fclose(fp);
    }
    fp = fopen("beds.txt", "w");
    if (fp) {
        ...
        while (b) {
            // 如果为空闲状态，写入占位符 none
            fprintf(fp, "%d %d %s %s %s %.2f\n", b->bedNumber, b->isOccupied, strlen(b->patientId) > 0 ? b->patientId : "none", b->wardType, b->bedType, b->price);
            b = b->next;
        }
        fclose(fp);
    }
